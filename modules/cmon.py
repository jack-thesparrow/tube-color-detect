...
known_colors = {
    ...
    "purple": [138, 183, 199],  # manually tuned
...
# === Parameters ===
ROW_STEP = 4
MIN_ROW_PIXEL_COUNT = 30
MIN_SEGMENT_HEIGHT = 30
MIN_SEGMENT_AREA = 30  # avoid oversplitting small glitches
    ...
    for row in range(0, height, row_step):
        ...
        if best_color != last_color:
            if last_color is not None and last_color != "black":
                if row - start_row >= MIN_SEGMENT_HEIGHT:
                    segments.append((last_color, start_row, row))
            start_row = row
            last_color = best_color

    if last_color is not None and last_color != "black":
        if height - start_row >= MIN_SEGMENT_HEIGHT:
            segments.append((last_color, start_row, height))
    return segments


def merge_similar_segments(segments, hsv_lookup, threshold=12):
    merged = []
    for color, start, end in segments:
        if merged:
            last_color, last_start, last_end = merged[-1]
            hsv1 = np.array(hsv_lookup[last_color])
            hsv2 = np.array(hsv_lookup[color])
...
if __name__ == "__main__":
    ...
    for filename in sorted(os.listdir(tubes_dir)):
        if filename.lower().endswith(".png"):
            ...
            segments = merge_similar_segments(segments, known_colors, threshold=12)
